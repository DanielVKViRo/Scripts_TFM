...
################################################################################
# Frecuencia de tipo de invasión dividida por riqueza de recursos y kappa
################################################################################
# =========================
# Datos
# =========================
...
comparaciones <- df_all %>%
  ...
  summarise(
    p = wilcox.test(invasion ~ kappa)$p.value,
    .groups = "drop"
  ) %>%
...
stat_test <- df_long %>%
  ...
  wilcox_test(porcentaje ~ tipo_invasion) %>%
